...
This report contains the complete implementation and experimental analysis for the project: "Fake News Detection Using Transformer Models: A Comparative Study with Traditional Methods."
...
High speed, word-frequency focused
...
Deep contextual/semantic understanding
...
Observations: The loss decreased exponentially from $\approx 0.68$ to $\approx 0.03$.
...
Logistic Regression: Encountered $\approx 3\%$ error rate. These errors often stem from "adversarial" fake news that uses neutral, high-frequency "Real" keywords to mask misinformation.

BERT Transformer: Achieved a near-perfect classification in test batches. By utilizing Self-Attention, BERT looks at the entire sentence structure, identifying structural inconsistencies in fake news even when the vocabulary seems legitimate.
...
⚙️ Technical Methodology1. Data Representation & PreprocessingText Concatenation: The model analyzes the Title combined with the Body Text to capture the maximum available context.Tokenization: Uses BertTokenizerFast. Unlike simple splitting, this uses WordPiece tokenization, which handles out-of-vocabulary words by breaking them into sub-units.Truncation: Sequences are capped at 128 tokens. Research suggests that for fake news, the most critical stylistic markers are located in the headline and the lead paragraph.2. Model Architecture: BERT-BaseBidirectional Context: Unlike previous models that read text left-to-right, BERT reads the entire sequence at once.Attention Mechanism: The model calculates "Attention Scores" between every word in a sentence, allowing it to understand the relationship between a subject and a distant verb or adjective.🛠️ Installation & SetupPrerequisitesEnsure you have Python 3.9+ installed. The environment requires approximately 4GB of RAM for the Transformer model.1. Install Dependenciespip install pandas scikit-learn torch transformers accelerate matplotlib seaborn
2. Prepare DataPlace your True.csv and Fake.csv files inside a folder named data/ in the root directory.3. Run the Pipelinepython main.py
⚖️ Ethics & Responsible UseTransparency: Automated detection should be viewed as a "decision support" tool for human moderators.Bias Awareness: The model may reflect biases inherent in the ISOT dataset (e.g., specific political eras or sources).Interpretability: While BERT is highly accurate, it acts as a "black box." The included confusion matrices are the first step in understanding its decision-making logic.Author: Vishnu Teja GudipatiRiv ID: A0000072432Course: COMP-690-AH2 - Topics in Computer ScienceDate: May 2024
